...
public partial class Griffin : CharacterBody2D
{
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        if (IsInstanceValid(Target))
        {
            ...
            // --- ANIMACIÓN DE ATAQUE ---
            // Si está lo suficientemente cerca para "atacar", reproducimos default
            if (GlobalPosition.DistanceTo(Target.GlobalPosition) < 50f)
            {
        ...
        {
            // MODO NIDO
            // Si no está atacando, podrías poner una animación de "vuelo_normal" 
            // o simplemente dejar que siga en default si es un loop de vuelo.
            if (_sprite?.Animation != "default") 
            {
                _sprite?.Play("default");
            }
